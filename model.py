...
class DeepQNetwork(nn.Module):
    def __init__(self, ALPHA):
        ...
        self.optimiser = optim.RSMprop(self.parameters(), lr=ALPHA)
        self.loss = nn.MSELOSS()
        self.device = t.device('cuda:0' if t.cuda.is_available() else 'cpu') #If the GPU is available, use that, otherwise use the CPU
        self.to(self.device)
